...
def api_merchants(user: dict = Depends(require_auth)):
    user_id = user["id"]
    conn = get_conn()
    rows = conn.execute(
        """
        ...
        FROM transactions t
        LEFT JOIN merchant_dictionary d
          ON d.user_id = ? AND LOWER(d.merchant_key) = LOWER(t.merchant_raw)
        WHERE t.user_id = ? AND t.direction = 'debit'
...
def api_merchant_tag(payload: dict = Body(...), user: dict = Depends(require_auth)):
    user_id = user["id"]
    ...
    conn.execute(
        """
        INSERT INTO merchant_dictionary (user_id, merchant_key, category, is_user_tagged, applied_count)
        VALUES (?, ?, ?, 1, 0)
        ON CONFLICT(user_id, merchant_key) DO UPDATE SET
          category = excluded.category,
          is_user_tagged = 1
    """,
        (user_id, merchant_raw.lower(), category),
    )

    cur = conn.execute(
        "UPDATE transactions SET category = ? WHERE user_id = ? AND LOWER(merchant_raw) = LOWER(?)",
        (category, user_id, merchant_raw),
    )
    updated = cur.rowcount

    conn.execute(
        "UPDATE merchant_dictionary SET applied_count = ? WHERE user_id = ? AND merchant_key = ?",
        (updated, user_id, merchant_raw.lower()),
    )
    conn.commit()
    conn.close()
